...
of the host processor, as typed arrays are native-endian
	(https://lists.w3.org/Archives/Public/public-script-coord/2010AprJun/0048.html,
	https://cat-in-136.blogspot.com/2011/03/javascript-typed-array-use-native.html)
*/
  ...
  window.JSSpeccy.buildZ80 = function (opts) {
    ...
    */
    ...
    opcodeSwitch = function (runStringTable, fallbackTable, traps) {
      var action, address, clauses, i, opcode, relevantTraps, runString, trapCode, _i;
      ...
      for (i = _i = 0; 0 <= 0x100 ? _i < 0x100 : _i > 0x100; i = 0 <= 0x100 ? ++_i : --_i) {
        ...
        if (runString != null) {
          relevantTraps = (function () {
            ...
            _results = [];
            for (_j = 0, _len = traps.length; _j < _len; _j++) {
              _ref = traps[_j], address = _ref[0], opcode = _ref[1], action = _ref[2];
              if (opcode === i) {
                _results.push([address, action]);
              }
            }
            return _results;
          })();
          trapCode = (function () {
            var _j, _len, _ref, _results;
            _results = [];
            for (_j = 0, _len = relevantTraps.length; _j < _len; _j++) {
              _ref = relevantTraps[_j], address = _ref[0], action = _ref[1];
              _results.push("if (regPairs[" + rpPC + "] == " + ((address + 1) & 0xffff) + " && !(" + action + ")) break;");
            }
            return _results;
          })();
          clauses.push("case " + i + ":\n	" + (trapCode.join("\n")) + "\n	" + runString + "\n	break;");
