# FUSE Z80 Test Runner Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a standalone Node.js runner that executes the FUSE Z80 test suite against the `@asm80/emu` Z80 emulator and reports register, T-state, and memory-change mismatches.

**Architecture:** A single ES module `test/fuse-z80/runner.js` with three responsibilities: parsing `tests.in` and `tests.expected`, running each test through a freshly created Z80 CPU, and printing a pass/fail report. No QUnit integration — invoked via `npm run test:fuse`.

**Tech Stack:** Node.js (ES modules), `@asm80/emu` Z80 CPU (imported from `../../src/z80.js` at top of file), Node.js `fs` built-in only.

**Prerequisite:** `test/fuse-z80/tests.in` and `test/fuse-z80/tests.expected` already exist in the repository (present as untracked files). Do not modify them.

---

## Chunk 1: Parser for `tests.in`

**Files:**
- Create: `test/fuse-z80/runner.js`

### Task 1: Parse `tests.in`

- [ ] **Step 1: Create `runner.js` with all imports and `parseTestsIn`, plus a smoke test**

  Create `test/fuse-z80/runner.js` with:
...
- [ ] **Step 2: Run and verify output**
  ...
  ```bash
  node test/fuse-z80/runner.js
  ```

  Expected: prints the count of parsed tests (should be ~1300+) and the first test object:
  ```
  Parsed XXXX tests from tests.in
  First test: {
    "name": "00",
    "regs": { "af": 0, "bc": 0, ..., "pc": 0 },
    "special": { "i": 0, "r": 0, "iff1": 0, "iff2": 0, "im": 0, "halted": 0, "tstates": 1 },
...
- [ ] **Step 3: Commit**
  ...
  ```bash
  git add test/fuse-z80/runner.js
  git commit -m "feat(fuse): add tests.in parser"
  ```

---

## Chunk 2: Parser for `tests.expected`

**Files:**
- Modify: `test/fuse-z80/runner.js`

### Task 2: Parse `tests.expected`

- [ ] **Step 1: Add `parseTestsExpected` function after `parseTestsIn`**

  Insert before the smoke test block:
  ...
  const parseTestsExpected = (text) => {
    ...
    while (i < lines.length) {
      ...
      // Skip event lines: first token is a decimal timestamp (pure digits)
      while (i < lines.length) {
        const first = lines[i].trim().split(/\s+/)[0];
        if (/^\d+$/.test(first)) { i++; continue; }
        break;
      }
...
- [ ] **Step 2: Replace smoke test block with dual-parser smoke test**

  Replace the smoke test at the bottom of `runner.js` with:
...
- [ ] **Step 3: Run and verify**
  ...
  Expected (both counts identical, no ERROR line):
  ```
  Parsed XXXX inputs, XXXX expected results
  First expected: {
    "name": "00",
    "regs": { "af": 0, "bc": 0, ..., "pc": 1 },
    "special": { "i": 0, "r": 1, "iff1": 0, "iff2": 0, "im": 0, "halted": 0, "tstates": 4 },
    "memChanges": []
  }
  ```

  Note: `testsIn[0].special.tstates` will be `1` (FUSE sentinel); `testsExp[0].special.tstates` will be `4` (actual NOP cycle count). This is expected.

- [ ] **Step 4: Commit**

  ```bash
  git add test/fuse-z80/runner.js
  git commit -m "feat(fuse): add tests.expected parser"
  ```

---

## Chunk 3: Test executor and reporter

**Files:**
- Modify: `test/fuse-z80/runner.js`
- Modify: `package.json`

### Task 3: Run all tests and print report

**`regMap` key note:** `regMap` entries are `[label, expKey, gotValue]`. `expKey` is looked up in `expected.regs` first; if absent, in `expected.special`. Key sets are disjoint: `regs` has `{af, bc, de, hl, afAlt, bcAlt, deAlt, hlAlt, ix, iy, sp, pc}`, `special` has `{i, r, iff1, iff2, im, halted, tstates}`. No collision.

- [ ] **Step 1: Add `runTest` function after `parseTestsExpected`**
...
- [ ] **Step 2: Replace smoke test with `main` function**

  Replace the smoke test block at the bottom of `runner.js` with:
...
- [ ] **Step 3: Add `test:fuse` script to `package.json`**
...
- [ ] **Step 4: Run the full suite**
  ...
  ```
  FUSE Z80: 1308 passed, 0 failed
  ```

  If failures exist, review the first few. A small number (< 50) related to undocumented opcodes or MEMPTR-dependent tests is acceptable. If failures are in the hundreds, suspect a parsing or register-setup bug — investigate before committing.

- [ ] **Step 5: Commit**

  ```bash
  git add test/fuse-z80/runner.js package.json
  git commit -m "feat(fuse): add FUSE Z80 runner with register, T-state, and memory verification"
  ```
